    ...
    if (dir.isDir()) {
        ...
        foreach (const QByteArray &mimeTypeName, supportedMime) {
            if (dir.completeSuffix() == mimeTypeName) {
                img->loadFile(path);
                boxTab->insertTab(boxTab->count(), img, QIcon(":/icons/CoreImage.svg"), "Core Image");
        ...
        if (!dir.isExecutable()) {
           pad->openText(path);
           boxTab->insertTab(boxTab->count(), pad, QIcon(":/icons/CorePad.svg"), "Core Pad");
           boxTab->setCurrentIndex(boxTab->count());
           return;
        }
    }
...
void bookmarks::on_boklist_itemDoubleClicked(QTableWidgetItem *item)
{
    ...
    if (QFile(s).exists()) {
        ...
        messageEngine("The file or folder deleted, can't mount or doesn't exist.", "Warning");
    }
}
